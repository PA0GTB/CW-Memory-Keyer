...
//
/*
   Modification by Cor Struyk, PA0GTB, december 2018
   -----------------------------------------------
   ...
   Changed Library Morse, Febr 2019, PA0GTB
   Additional modification in Morse.cpp to define progsigns BK, KA and SK, 
   Progsigns are now correct written on LCD Display 
    
   Additional modification by Cor Struyk, PA0GTB and Edwin Arts, PA7FRN, october 2017
   Changing the behavior of the 20x4 LCD Display  " position files changed December 2018"
   ...
   the place where this Sketch can be found.
...
const int lcd_address = 0x3F; // define the used I2C LCD Address
...
const String ready = "CW Keyer v1.5 Ready";
...
int key_speed = 18;      // initial start_speed;
    ...
    switch (c)  // Case Statement to determine which key was pressed
    {
      ...
      default:  // Otherwise do the default action, which is to continue on
        ...
        {  
          ...
          // If we're not doing the Macro thing, check if we use programmed Progsigns
             switch (c)
             {
             case '=':
             lcd.print(" BT");
          ...
          // If we're not doing those things,  just print and send all normal characters
             default:  
